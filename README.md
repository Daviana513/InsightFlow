...
本地模式已经可以读取真实图片目录与 CSV、自动检查记录数/图片数/缺图和必要字段，创建稳定记录 ID，并真正运行数据索引预处理。任务、预处理结果与人工审核记录保存在项目旁的 `.insightflow/insightflow.db`。服务只监听本机地址；启动 Local Agent 后，正式作品集网页或本机网页都能连接它，原始图片不会上传到公开站点。
...
本地模式现在可以使用独立的 OpenCLIP 环境运行 `ViT-B-32 + Logistic Regression` 分类器，逐张保存概率，并按任务创建时冻结的 `p >= 0.10` 生成真实候选集。OpenCLIP 页同时提供独立的“选择训练图片”入口：当“信息图”和“非信息图”各至少标注 20 张、且各覆盖至少 5 个不同帖子后，页面才会出现“训练新模型”按钮。训练在电脑本地完成并另存模型版本，下一次筛选自动使用新模型；正式研究建议每类至少 100 张。人工标注不会改变已经完成的本次筛选结果。
